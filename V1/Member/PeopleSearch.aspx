...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <style>
        .hpanel { margin-bottom: 5px !important; }
        .user-name { color: #337ab7 !important; text-decoration: none; cursor: pointer; }
        .user-name:hover { color: #23527c !important; text-decoration: underline; }
        .member-panel-body { border-radius: 10px !important; margin-bottom: 0px !important; }
        .row.no-gutter { margin-left: 0; margin-right: 0; }
        .row.no-gutter [class*="col-"] { padding-left: 1px; padding-right: 1px; }
        .friendsGrid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; width: 100%; padding: 0; list-style: none; }
        .friendItem { padding: 10px; border: 1px solid #ccc; text-align: center; }
        @media screen and (max-width: 768px) { .friendsGrid { grid-template-columns: 1fr; } }
        p { margin: 0px !important; }
        .fa-pending-color { color: gainsboro; }
        .fa-approved-color { color: #63CB31; }
                                        ...
                                        <div class="panel-body member-panel-body">
                                            <div class="clearfix">
                                                <img class="img-circle img-small pull-left m-r-md" 
                                                     src="<%=profilePhotoFolder%><%# string.IsNullOrEmpty(Eval("ProfileImage") as string) ? "icons8-customer-64.png" : Eval("ProfileImage")%>" />
                                                <div class="media-body">
                                                    <p style="font-size: 16px;">
                                                        ...
                                                        <div class="text-muted">
                                                            <asp:Literal ID="CityState" runat="server" Text='<%# Eval("CityState") %>'></asp:Literal>
                                                        </div>
                                                        <div class="text-muted">
                                                            <asp:Literal ID="TeamName" runat="server" Text='<%# Eval("TeamName") %>'></asp:Literal>
                                                            &nbsp;
                                                            <asp:Literal ID="ProfileTitle" runat="server" Text='<%# Eval("ProfileTitle") %>'></asp:Literal>
                                                        </div>

                                                        <button data-id="<%#Eval("UserId")%>" data-action="remove" onclick="UpdateConnection(this); return false;" 
                                                                class="btn btn-default pull-right add-button" <%=_hideConnectionButton%>>
                                                            Add Connection
                                                        </button>
                                                    </p>
                                                </div>
                                            ...
                                            <div class="social-content m-t-sm">
                                                <div class="text-muted">
                                                    <asp:Literal ID="ProfileDescription" runat="server" Text='<%# Eval("ProfileDescription") %>'></asp:Literal>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
