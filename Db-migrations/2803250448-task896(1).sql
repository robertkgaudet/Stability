ALTER PROCEDURE [dbo].[MapStabilityLocations]  
    ...
    @LocationParentTypeId UNIQUEIDENTIFIER = NULL  
AS  
BEGIN  
    SET NOCOUNT ON;  
    ...
    SET @result = (  
        SELECT TOP 500  
            ...
            LPT.[LocationParentTypeId] AS [properties.LocationParentType],  -- GUID instead of Name  
            CASE  
        ...
        -- Added Join for LocationParentType if applicable  
        LEFT JOIN LocationParentType LPT ON A.AddressId = LPT.LocationParentTypeId  
        WHERE A.Longitude IS NOT NULL  
            ...
            AND (@LocationParentTypeId IS NULL OR LPT.LocationParentTypeId = @LocationParentTypeId)  -- Filter using GUID  
        FOR JSON PATH  
    );  
